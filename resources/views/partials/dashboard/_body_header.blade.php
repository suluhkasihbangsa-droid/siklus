        ...
        <li class="nav-item dropdown">
          ...
          <a class="nav-link py-0 d-flex align-items-center" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
              ...
              @if (Auth::user()->hasRole('admin'))
                  {{-- UNTUK ADMIN: Tampilkan semua gambar asli agar theme-switcher berfungsi --}}
              ...
              @else
                  {{-- UNTUK ROLE LAIN: Tampilkan satu gambar dinamis --}}
                  @php
                      $user = Auth::user();
                      $gender = $user->userProfile->gender ?? 'L'; // Default ke Laki-laki 'L' jika gender kosong
                      $avatar = asset('images/shapes/05.png'); // Gambar fallback

                      if ($user->hasRole('dokter')) {
                          $avatar = ($gender == 'P') ? asset('images/avatars/4.jpg') : asset('images/avatars/3.jpg');
                      } elseif ($user->hasRole(['user', 'koorUser'])) {
                          $avatar = ($gender == 'P') ? asset('images/avatars/2.jpg') : asset('images/avatars/1.jpg');
                      }
                  @endphp
                  <img src="{{ $avatar }}" alt="User-Profile" class="img-fluid avatar avatar-50 avatar-rounded">
              @endif

              {{-- Bagian Caption Nama dan Role tidak berubah --}}
