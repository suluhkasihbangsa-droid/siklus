    ...
    <table class="table table-striped" role="grid">
        <thead>
            <tr class="ligth">
                <th scope="col">#</th>
                <th scope="col">
                    {{-- Tombol Sortir untuk ID --}}
                    <a href="{{ route('users.index', ['sort_by' => 'id', 'sort_direction' => ($sortBy == 'id' && $sortDirection == 'asc') ? 'desc' : 'asc']) }}">
                        ID
                        @if ($sortBy == 'id')
                ...
                <th scope="col">
                    ...
                    <a href="{{ route('users.index', ['sort_by' => 'first_name', 'sort_direction' => ($sortBy == 'first_name' && $sortDirection == 'asc') ? 'desc' : 'asc']) }}">
                        Nama
                ...
                <th scope="col">Organisasi Ditugaskan</th>
            ...
            @forelse ($users as $user)
                <tr>
                    ...
                    <td>
                        ...
                        {{-- UBAH 1: Tambah No. HP --}}
                        <br>
                        <small class="text-muted">{{ $user->userProfile->phone_number ?? '-' }}</small>
                    </td>
                    <td>
                        @php
                            // UBAH 2: Logika Badge Warna-Warni untuk Role
                            $role = $user->roles->first()->name ?? '';
                            $badgeColor = 'bg-secondary'; // Warna default
                            if ($role === 'admin') $badgeColor = 'bg-danger';
                            if ($role === 'koorUser') $badgeColor = 'bg-warning text-dark';
                            if ($role === 'dokter') $badgeColor = 'bg-info';
                            if ($role === 'user') $badgeColor = 'bg-primary';
                        @endphp
                        <span class="badge {{ $badgeColor }}">{{ $role }}</span>
                    </td>
                    <td>
                        {{-- UBAH 3: Tampilan Organisasi dengan Teks --}}
                        {{ $user->organisasis->pluck('nama_organisasi')->implode(', ') ?: '-' }}
                    </td>
                    <td>
                        {{-- UBAH 4: Ubah Tampilan Tombol Aksi --}}
                        <div class="d-flex align-items-center list-user-action">
                            @if(Auth::user()->hasAnyRole(['admin', 'koorUser']) || Auth::id() == $user->id)
                                <a class="btn btn-sm btn-icon btn-warning" href="{{ route('users.edit', $user->id) }}" title="Edit">
                                    <span class="btn-inner"><svg width="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M11.4925 2.78906H7.75349C4.67849 2.78906 2.75049 4.96606 2.75049 8.04806V16.3621C2.75049 19.4441 4.66949 21.6211 7.75349 21.6211H16.5775C19.6625 21.6211 21.5815 19.4441 21.5815 16.3621V12.3341" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M8.82812 10.921L16.3011 3.44799C17.2321 2.51799 18.7411 2.51799 19.6721 3.44799L20.8891 4.66499C21.8201 5.59599 21.8201 7.10599 20.8891 8.03599L13.3801 15.545C12.9731 15.952 12.4211 16.181 11.8451 16.181H8.09912L8.19312 12.401C8.20712 11.845 8.43412 11.315 8.82812 10.921Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path><path d="M15.1655 4.60254L19.7315 9.16854" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></span>
                                </a>
                            @endif
